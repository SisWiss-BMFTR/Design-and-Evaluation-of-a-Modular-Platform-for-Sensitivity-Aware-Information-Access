...
\begin{figure}[htbp]
  \centering
  \resizebox{0.98\textwidth}{!}{\input{figures/architecture/system_architecture}}
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.99\textwidth}{!}{\input{figures/architecture/guard_execution_pipeline}}
...
These controls use patterns, exact-value inventories, policy objects, and mode-dependent branches. Switch state, trigger execution, guard action, and attack-outcome reduction are recorded as distinct observations. Guard telemetry therefore cannot by itself establish a reduction in the attack-specific outcome. These distinctions are part of the empirical interpretation rather than hidden implementation details.
