...
\begin{figure}[H]
  \centering
  \resizebox{0.93\linewidth}{!}{\input{figures/architecture/threat_model}}
...
An attack-specific security numerator is calculated only over public and internal conversations, giving 150 unauthorised records per attack--mode--stage cell. Protected conversations have a separate denominator of 75. Protected failures are recorded as positive-control task failures and excluded from the security-success count. The positive control is narrow: it tests the expected protected answer for that attack, not general helpfulness or deployment utility.
...
\begin{figure}[p]
  \centering
  \resizebox{0.98\textwidth}{!}{\input{figures/architecture/evidence_hierarchy}}
