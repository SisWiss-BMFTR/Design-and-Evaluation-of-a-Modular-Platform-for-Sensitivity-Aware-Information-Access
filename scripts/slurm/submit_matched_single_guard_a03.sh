...
run_root="$PROJECT_ROOT/outputs/experiments/matched_single_guard_ablations/E03_A03_access_change_memory_clear_$run_tag"
...
cp -a scripts/slurm/run_matched_single_guard_a03.sbatch "$source_root/scripts/slurm/"
cp -a scripts/slurm/submit_matched_single_guard_a03.sh "$source_root/scripts/slurm/"
...
from evaluation2.run_access_level_downgrade_task import (
    TARGETS,
    build_attack_prompt,
    build_seed_prompt,
    warmup_prompts_for_history_length,
)
    ...
    "targets": [
        {
            **asdict(target),
            "conversations": {
                str(length): {
                    "warmups": warmup_prompts_for_history_length(length),
                    "seed": build_seed_prompt(target),
...
arm_a = {
    "access_change_memory_clear": False,
    "output_leakage_verifier": False,
}
arm_b = dict(arm_a, access_change_memory_clear=True)
differences = [key for key in arm_a if arm_a[key] != arm_b[key]]
if differences != ["access_change_memory_clear"]:
...
manifest = {
    ...
    "attack": "A03_access_level_downgrade_memory_leakage",
    "intervention": "access_change_memory_clear",
    "git_commit": commit,
    ...
    "access_after_levels": ["public", "internal", "protected"],
    ...
    "expected_model_calls_per_arm": 1800,
    "expected_model_calls_total": 3600,
}
...
submission=$(sbatch \
  --job-name="a03-memory-clear" \
  --output="$run_root/slurm/a03_%A_%a.out" \
  --error="$run_root/slurm/a03_%A_%a.err" \
  --export="ALL,PROJECT_ROOT=$PROJECT_ROOT,RUN_ROOT=$run_root,SOURCE_ROOT=$source_root" \
  "$source_root/scripts/slurm/run_matched_single_guard_a03.sbatch")
