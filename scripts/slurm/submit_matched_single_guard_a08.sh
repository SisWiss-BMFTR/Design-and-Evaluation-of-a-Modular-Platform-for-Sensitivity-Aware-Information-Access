...
run_root="$PROJECT_ROOT/outputs/experiments/matched_single_guard_ablations/E08_A08_embedding_probe_guard_$run_tag"
...
cp -a scripts/slurm/run_matched_single_guard_a08.sbatch "$source_root/scripts/slurm/"
cp -a scripts/slurm/submit_matched_single_guard_a08.sh "$source_root/scripts/slurm/"
...
from evaluation2.run_embedding_side_leakage import (
    DATASET, TARGETS, build_attack_prompt, warmup_prompts_for_length,
)
        ...
        **asdict(target),
        "conversations": {
            str(length): {
                ...
                "attack": build_attack_prompt(target, "neutral"),
            }
            for length in (1, 3, 5)
        },
...
arm_b = dict(arm_a, embedding_probe_guard=True)
differences = [key for key in arm_a if arm_a[key] != arm_b[key]]
if differences != ["embedding_probe_guard"]:
...
manifest = {
    ...
    "attack": "A08_embedding_side_leakage",
    "intervention": "embedding_probe_guard",
    "git_commit": commit,
...
submission=$(sbatch \
  --job-name="a08-embedding-guard" \
  --output="$run_root/slurm/a08_%A_%a.out" \
  --error="$run_root/slurm/a08_%A_%a.err" \
  --export="ALL,PROJECT_ROOT=$PROJECT_ROOT,RUN_ROOT=$run_root,SOURCE_ROOT=$source_root" \
  "$source_root/scripts/slurm/run_matched_single_guard_a08.sbatch")
job_id="${submission##* }"
"$PROJECT_ROOT/env/rag/bin/python3" - "$run_root" "$job_id" <<'PY'
import json, sys
from pathlib import Path
Path(sys.argv[1], "submission.json").write_text(
    json.dumps({"slurm_job_id": sys.argv[2]}, indent=2) + "\n", encoding="utf-8"
)
PY
echo "$submission"
