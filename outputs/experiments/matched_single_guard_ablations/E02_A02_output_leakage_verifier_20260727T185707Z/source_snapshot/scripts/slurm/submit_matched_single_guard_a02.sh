...
run_root="$PROJECT_ROOT/outputs/experiments/matched_single_guard_ablations/E02_A02_output_leakage_verifier_$run_tag"
source_root="$run_root/source_snapshot"

mkdir -p "$run_root/slurm" "$source_root/data" "$source_root/scripts/slurm" "$source_root/scripts"
...
cp -a scripts/rescore_a02_policy_aware.py "$source_root/scripts/"
cp -a scripts/slurm/run_matched_single_guard_a02.sbatch "$source_root/scripts/slurm/"
cp -a scripts/slurm/submit_matched_single_guard_a02.sh "$source_root/scripts/slurm/"
...
from evaluation2.run_multiturn_row_construction_batch import (
    SCORABLE_FIELDS,
    build_conversation_prompts,
    load_targets,
)

targets = load_targets(["all"])
prompt_manifest = {
    "attack": "A02",
    ...
    "targets": [
        {
            **asdict(target),
            "conversations": {
                str(length): build_conversation_prompts(target, length, "neutral")
                for length in (1, 3, 5)
            },
        }
...
arm_b = dict(arm_a, output_leakage_verifier=True)
...
manifest = {
    ...
    "attack": "A02_multiturn_row_construction",
    ...
    "prompt_style": "neutral",
    "scorer": {
        "primary": "a02-policy-aware-v1",
        "path": "scripts/rescore_a02_policy_aware.py",
        "known_prompt_fields_excluded": ["Rezeptur-ID", "Rohstoff"],
    },
...
submission=$(sbatch \
  --job-name="a02-output-guard" \
  --output="$run_root/slurm/a02_%A_%a.out" \
  --error="$run_root/slurm/a02_%A_%a.err" \
  --export="ALL,PROJECT_ROOT=$PROJECT_ROOT,RUN_ROOT=$run_root,SOURCE_ROOT=$source_root" \
  "$source_root/scripts/slurm/run_matched_single_guard_a02.sbatch")
